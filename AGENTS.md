...
- Services: Encapsulate domain logic (e.g., GradeService). In React, access services only via react-query hooks; never call services directly from components. Avoid direct DB usage outside service/data layers.
- UI Components: Implement reusable components that have local styling, do not create inline styling, always have it in a seperate CSS module.
...
## 9. How to propose schema or API changes
